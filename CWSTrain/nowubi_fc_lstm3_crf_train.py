...
tf.app.flags.DEFINE_string('train_data_path', "Corpora/pku", 'Training data dir')
tf.app.flags.DEFINE_string('test_data_path', "Corpora/pku", 'Test data dir')
tf.app.flags.DEFINE_string('log_dir', "Logs/seg_logs", 'The log dir')
tf.app.flags.DEFINE_string("word2vec_path", "char_vec.txt", "the word2vec data path")
...
def inference_fc(X_char, X_pinyin, weights, bias):
    word_vectors = tf.nn.embedding_lookup(WORDS_char, X_char)
    pinyin_vectors = tf.nn.embedding_lookup(WORDS_pinyin, X_pinyin)
    # [batch_size, 80, 100]

    temp_vectors = tf.concat([word_vectors, pinyin_vectors], 2)
    # [batch_size, 80, 300]

    reshaped_vectors = tf.reshape(temp_vectors, [-1, FLAGS.embedding_size * 2])
...
def main(unused_argv):
    ...
    with graph.as_default():
        inp_char, inp_pinyin, hidden_W, hidden_b, hidden_W_fc, hidden_b_fc = initialization(FLAGS.word2vec_path, FLAGS.pinyin2vec_path)
        ...
        final_vectors = inference_fc(X_char, X_pinyin, hidden_W_fc, hidden_b_fc)
        ...
        test_final_vectors = inference_fc(inp_char, inp_pinyin, hidden_W_fc, hidden_b_fc)
                ...
                try:
                    ...
                    if (step + 1) % 1000 == 0:
                        ...
                        precision = test_evaluate(sess, test_unary_score,
                                      test_sequence_length, trainsMatrix, inp_char, inp_pinyin, tX_char, tX_pinyin, tY_char)
