...
def inference(X, final_vectors, weights, bias, reuse = None, trainMode = True):
    ...
    with tf.variable_scope("rnn_fwbw_4", reuse = reuse) as scope:
        forward_output_4, _ = tf.nn.dynamic_rnn(
            tf.contrib.rnn.LSTMCell(FLAGS.num_hidden, reuse = reuse),
            output_3,
            dtype = tf.float32,
            sequence_length = length,
            scope = "RNN_forward_4")
        backward_output_4_, _ = tf.nn.dynamic_rnn(
            tf.contrib.rnn.LSTMCell(FLAGS.num_hidden, reuse = reuse),
            inputs = tf.reverse_sequence(output_3,
                                        length_64,
                                        seq_dim = 1),
            dtype = tf.float32,
            sequence_length = length,
            scope = "RNN_backword_4")

    backward_output_4 = tf.reverse_sequence(backward_output_4_,
                                            length_64,
                                            seq_dim = 1)
