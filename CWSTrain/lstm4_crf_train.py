...
def inference(X, weights, bias, reuse = None, trainMode = True):
    ...
    with tf.variable_scope("rnn_fwbw_2", reuse = reuse) as scope:
        forward_output_2, _ = tf.nn.dynamic_rnn(tf.contrib.rnn.LSTMCell(FLAGS.num_hidden, reuse = reuse),
                output_1,
                dtype = tf.float32,
                sequence_length = length,
                scope = "RNN_forward_2")
        backward_output_2_, _ = tf.nn.dynamic_rnn(tf.contrib.rnn.LSTMCell(FLAGS.num_hidden, reuse = reuse),
        inputs = tf.reverse_sequence(output_1, length_64, seq_dim = 1),
        dtype = tf.float32,
        sequence_length = length,
        scope = "RNN_backward_2")

    backward_output_2 = tf.reverse_sequence(backward_output_2_, length_64, seq_dim = 1)

    output_2 = forward_output_2 + backward_output_2

    with tf.variable_scope("rnn_fwbw_3", reuse = reuse) as scope:
        forward_output_3, _ = tf.nn.dynamic_rnn(tf.contrib.rnn.LSTMCell(FLAGS.num_hidden, reuse = reuse),
                output_2,
                dtype = tf.float32,
                sequence_length = length,
                scope = "RNN_forward_3")
        backward_output_3_, _ = tf.nn.dynamic_rnn(tf.contrib.rnn.LSTMCell(FLAGS.num_hidden, reuse = reuse),
                inputs = tf.reverse_sequence(output_2, length_64, seq_dim = 1),
                dtype = tf.float32,
                sequence_length = length,
                scope = "RNN_backward_3")

    backward_output_3 = tf.reverse_sequence(backward_output_3_, length_64, seq_dim = 1)

    output_3 = forward_output_3 + backward_output_3

    with tf.variable_scope("rnn_fwbw_4", reuse = reuse) as scope:
        forward_output_4, _ = tf.nn.dynamic_rnn(tf.contrib.rnn.LSTMCell(FLAGS.num_hidden, reuse = reuse),
                output_3,
                dtype = tf.float32,
                sequence_length = length,
                scope = "RNN_forward_4")
        backward_output_4_, _ = tf.nn.dynamic_rnn(tf.contrib.rnn.LSTMCell(FLAGS.num_hidden, reuse = reuse),
                inputs = tf.reverse_sequence(output_3, length_64, seq_dim = 1),
                dtype = tf.float32,
                sequence_length = length,
                scope = "RNN_backward_4")

    backward_output_4 = tf.reverse_sequence(backward_output_4_, length_64, seq_dim = 1)

    output_4 = tf.concat([forward_output_4, backward_output_4], 2)

    output = tf.reshape(output_4, [-1, FLAGS.num_hidden * 2])
    if trainMode:
        ...
        with sv.managed_session(master = '') as sess:
            ...
            for step in range(training_steps):
                ...
                try:
                    ...
                    if (step + 1) % 100 == 0:
                        precision = test_evaluate(sess, test_unary_score, test_sequence_length, trainsMatrix, inp, tX, tY)
