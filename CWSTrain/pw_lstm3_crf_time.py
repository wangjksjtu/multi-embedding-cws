...
def initialization(c2vPath, p2vPath, w2vPath):
    ...
    return inp_char, inp_pinyin, inp_wubi, hidden_W_char, hidden_b_char, hidden_W_pinyin, \
           hidden_b_pinyin, hidden_W_wubi, hidden_b_wubi
...
def inference(X, weights, bias, reuse = None, trainMode = True, type="char"):
    ...
    output1 = forward_output + backward_output
    if type == "char":
        with tf.variable_scope("rnn_fwbw_2", reuse = reuse) as scope:
            ...
            backward_output_2_, _ = tf.nn.dynamic_rnn(
                tf.contrib.rnn.LSTMCell(FLAGS.num_hidden, reuse = reuse),
                inputs = tf.reverse_sequence(output1,
                                           length_64,
                                           seq_dim = 1),
                dtype = tf.float32,
                sequence_length = length,
    ...
    output2 = forward_output_2 + backward_output_2
    # [batch_size, 80, 200]
    if type == "char":
        with tf.variable_scope("rnn_fwbw_3", reuse = reuse) as scope:
            ...
            backward_output_3_, _ = tf.nn.dynamic_rnn(
                tf.contrib.rnn.LSTMCell(FLAGS.num_hidden, reuse = reuse),
                inputs = tf.reverse_sequence(output2,
                                           length_64,
                                           seq_dim = 1),
                dtype = tf.float32,
                sequence_length = length,
                scope = "RNN_backward_3")
    ...
    output = tf.concat([forward_output_3, backward_output_3], 2)
...
def main(unused_argv):
    ...
    with graph.as_default():
        initial_args = initialization(FLAGS.word2vec_path, FLAGS.pinyin2vec_path, FLAGS.wubi2vec_path)
        inp_char, inp_pinyin, inp_wubi = initial_args[0:3]
        ...
        P_char, sequence_length_char = inference(X_char, initial_args[3], initial_args[4], type="char")
        P_pinyin, sequence_length_pinyin = inference(X_pinyin, initial_args[5], initial_args[6], type="pinyin")
        P_wubi, sequence_length_wubi = inference(X_wubi, initial_args[7], initial_args[8], type="wubi")

        P = P_char + P_pinyin + P_wubi
        ...
        test_unary_score_char, test_sequence_length_char = inference(inp_char,initial_args[3], initial_args[4],
                                                                     reuse = True, trainMode = False) #fix
        test_unary_score_pinyin, test_sequence_length_pinyin = inference(inp_pinyin,initial_args[5], initial_args[6],
                                                                         reuse = True, trainMode = False, type = 'pinyin') #fix
        test_unary_score_wubi, test_sequence_length_wubi = inference(inp_wubi,initial_args[7], initial_args[8],
                                                                     reuse = True, trainMode = False, type = 'wubi') #fix

        test_unary_score = test_unary_score_char + test_unary_score_pinyin + test_unary_score_wubi
            ...
            for step in range(training_steps):
                ...
                try:
                    ...
                    if (step + 1) % 100 == 0:
                        ...
                        logs.write("Accuracy: %.3f%%\n" % precision)
                        if precision >= 95:
                            print("Time: ", (time.time() - time_beginning))
                            logs.write("Time: %ds" % (time.time() - time_beginning))
                    logs.close()
